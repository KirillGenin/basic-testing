...
describe('simpleCalculator tests', () => {
  ...
  test('should divide two numbers', () => {
    const result = simpleCalculator({ a: 18, b: 9, action: Action.Divide });
  ...
  test('should return null for invalid action', () => {
    const result = simpleCalculator({ a: 10, b: 2, action: 'JavaScript' });
    expect(result).toBeNull();
  });

  test('should return null for invalid arguments', () => {
    const result = simpleCalculator({ a: 'one', b: 'two', action: Action.Add });
    expect(result).toBeNull();
  });
});